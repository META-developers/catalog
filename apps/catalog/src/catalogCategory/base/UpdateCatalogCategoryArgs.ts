/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { CatalogCategoryWhereUniqueInput } from "./CatalogCategoryWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { CatalogCategoryUpdateInput } from "./CatalogCategoryUpdateInput";

@ArgsType()
class UpdateCatalogCategoryArgs {
  @ApiProperty({
    required: true,
    type: () => CatalogCategoryWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => CatalogCategoryWhereUniqueInput)
  @Field(() => CatalogCategoryWhereUniqueInput, { nullable: false })
  where!: CatalogCategoryWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => CatalogCategoryUpdateInput,
  })
  @ValidateNested()
  @Type(() => CatalogCategoryUpdateInput)
  @Field(() => CatalogCategoryUpdateInput, { nullable: false })
  data!: CatalogCategoryUpdateInput;
}

export { UpdateCatalogCategoryArgs as UpdateCatalogCategoryArgs };
